...
# Web server for HTML files and API
web_app = None
web_runner = None
...
async def ensure_user(user_id: int):
    await db.register_user(settings.db_path, user_id)
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Get web app URL (you'll need to set this in your environment)
    ...
    if is_admin(message.from_user.id):
        keyboard.keyboard.append([KeyboardButton(text="🛠 Admin panel"), KeyboardButton(text="📊 Statistics")])
    
    await message.answer(
        "Добро пожаловать! Нажмите кнопку, чтобы открыть приложение и выбрать подарок.",
...
async def admin_add_promo(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AddPromo.preview_photo)
    await callback.message.answer("Send preview image for the card (photo). You can skip with /skip.")
    await callback.answer()


@router.message(AddPromo.preview_photo, F.photo)
@admin_only
async def addpromo_preview_photo(message: Message, state: FSMContext):
...
@router.message(AddPromo.preview_photo, F.text == "/skip")
@admin_only
async def addpromo_skip_preview_photo(message: Message, state: FSMContext):
...
async def addpromo_description(message: Message, state: FSMContext):
    ...
    await state.set_state(AddPromo.photo)
    await message.answer("Send promotion image for modal (photo). You can skip with /skip.")


@router.message(AddPromo.photo, F.photo)
@admin_only
async def addpromo_photo(message: Message, state: FSMContext):
    file_id = message.photo[-1].file_id
...
@router.message(AddPromo.photo, F.text == "/skip")
@admin_only
async def addpromo_skip_photo(message: Message, state: FSMContext):
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="Image", callback_data="admin:edit_field:image_file_id"),
            ],
...
async def editpromo_choose_field(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Send new value:")
    await callback.answer()
...
async def serve_html(request):
    ...
    # Also inject Telegram WebApp script
